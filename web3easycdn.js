...
async function checkSubscriptionStatus() {
    ...
    if (!userId) {
        window.location.href = '/buy-pro-plan'; // Редирект, если user_id отсутствует
        return;
    }
    ...
    try {
        ...
        if (error || !subscription || subscription.status === "Expired") {
            window.location.href = '/buy-pro-plan'; // Редирект, если подписка истекла
        } else if (subscription.status === "Active") {
            document.body.style.display = "block"; // Показываем содержимое страницы
        }
    } catch (error) {
